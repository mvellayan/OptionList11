...
BEGIN
    ...
    select expiry, op_iv, op_tv, cl_iv, cl_tv into l_expiry, l_op_iv, l_op_tv, l_cl_iv,  l_cl_tv from model where model_id = p_model_id;

    truncate table logs;
    insert into logs (log) values ( CONCAT_WS(" ", l_expiry, l_op_iv, l_op_tv, l_cl_iv,  l_cl_tv));
